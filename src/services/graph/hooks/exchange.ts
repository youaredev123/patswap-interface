...
export function useRikoPrice(variables = undefined, swrConfig: SWRConfiguration = undefined) {
  const { data } = useSWR(['rikoPrice', JSON.stringify(variables)], () => getRikoPrice(variables), swrConfig)
  return data
}
