...
import { BAR_ADDRESS, ZERO } from '@amblade/patswap-sdk'
import React, { useEffect, useState } from 'react'
import { SUSHI, XSUSHI, RIKO, XRIKO } from '../../config/tokens'
...
import useRikoBar from '../../hooks/useRikoBar'
import { useSushiPrice } from '../../services/graph'
import { useRikoPrice } from '../../services/graph'
...
export default function Stake() {
  ...
  const rikoBalance = useTokenBalance(account ?? undefined, RIKO[ChainId.MAINNET])
  const xRikoBalance = useTokenBalance(account ?? undefined, XRIKO[ChainId.MAINNET])

  const rikoPrice = useRikoPrice()

  const { enter, leave } = useRikoBar()

  const { data } = useSWR(`{bar(id: "0xc4862254eF722d3EFF26DDA1534A13bCb7BbE44f") {ratio, totalSupply}}`, fetcher)

  const xRikoPerRiko = parseFloat(data?.bar?.ratio)
  ...
  const balance = activeTab === 0 ? rikoBalance : xRikoBalance
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const apr = (((results[1].volumeUSD * 0.05) / data?.bar?.totalSupply) * 365) / (data?.bar?.ratio * rikoPrice)

      setApr(apr)
    }
    fetchData()
  }, [data?.bar?.ratio, data?.bar?.totalSupply, rikoPrice])

  return (
    <Container id="bar-page" className="py-4 md:py-8 lg:py-12" maxWidth="full">
      <Head>
        <title key="title">Stake | Riko</title>
        <meta
          key="description"
          name="description"
          content="Stake RIKO in return for xRIKO, an interest bearing and fungible ERC20 token designed to share revenue generated by all RIKO products."
        />
        <meta key="twitter:url" name="twitter:url" content="https://app.sushi.com/stake" />
        <meta key="twitter:title" name="twitter:title" content="STAKE RIKO" />
        <meta
        ...
        <meta key="og:title" property="og:title" content="STAKE RIKO" />
          ...
          <div className="flex flex-col w-full max-w-xl mt-auto mb-2">
            <div className="flex max-w-lg">
              <div className="self-end mb-3 text-lg font-bold md:text-2xl text-high-emphesis md:mb-7">
                {i18n._(t`Maximize yield by staking RIKO for xRIKO`)}
              </div>
            ...
            <div className="max-w-lg pr-3 mb-2 text-sm leading-5 text-gray-500 md:text-base md:mb-4 md:pr-0">
              {i18n._(t`For every swap on the exchange on every chain, 0.05% of the swap fees are distributed as RIKO
                                proportional to your share of the RikoBar. When your RIKO is staked into the RikoBar, you receive
                                xRIKO in return for voting rights and a fully composable token that can interact with other protocols.
                                Your xRIKO is continuously compounding, when you unstake you will receive all the originally deposited
                                RIKO and any additional from fees.`)}
            </div>
            {/* <div className="flex">
              ...
              <div className="w-full max-w-xl px-3 pt-2 pb-6 rounded bg-dark-900 md:pb-9 md:pt-4 md:px-8">
                ...
                <div className="flex items-center justify-between w-full mt-6">
                  <p className="font-bold text-large md:text-2xl text-high-emphesis">
                    {activeTab === 0 ? i18n._(t`Stake RIKO`) : i18n._(t`Unstake`)}
                  </p>
                  <div className="border-gradient-r-pink-red-light-brown-dark-pink-red border-transparent border-solid border rounded-3xl px-4 md:px-3.5 py-1.5 md:py-0.5 text-high-emphesis text-xs font-medium md:text-base md:font-normal">
                    {`1 xRIKO = ${xRikoPerRiko.toFixed(4)} RIKO`}
                  </div>
                </div>
                ...
                <div className="relative w-full h-0 pointer-events-none bottom-14">
                  ...
                  >
                    <div className="flex space-x-2 ">
                      ...
                      >
                        {`${input ? input : '0'} ${activeTab === 0 ? '' : 'x'}RIKO`}
                      </p>
                    </div>
              ...
              <div className="flex flex-wrap">
                  ...
                  <div className="flex items-center space-x-4">
                    ...
                    <div className="flex flex-col justify-center">
                      <p className="text-sm font-bold md:text-lg text-high-emphesis">
                        {xRikoBalance ? xRikoBalance.toSignificant(4) : '-'}
                      </p>
                      <p className="text-sm md:text-base text-primary">xRIKO</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-col flex-grow">
                  ...
                  <div className="flex items-center ml-8 space-x-4 md:ml-0">
                    ...
                    <div className="flex flex-col justify-center">
                      <p className="text-sm font-bold md:text-lg text-high-emphesis">
                        {rikoBalance ? rikoBalance.toSignificant(4) : '-'}
                      </p>
                      <p className="text-sm md:text-base text-primary">RIKO</p>
                    </div>
                  </div>
                </div>
