...
import RIKOBAR_ABI from '../constants/abis/rikobar.json'
...
import RIKO_ABI from '../constants/abis/riko.json'
...
export function useRikoContract(withSignerIfPossible = true): Contract | null {
  const { chainId } = useActiveWeb3React()
  return useContract(chainId && RIKO_ADDRESS[chainId], RIKO_ABI, withSignerIfPossible)
}
...
export function useRikoBarContract(withSignerIfPossible?: boolean): Contract | null {
  const { chainId } = useActiveWeb3React()
  return useContract(chainId && RIKOBAR_ADDRESS[chainId], RIKOBAR_ABI, withSignerIfPossible)
}
